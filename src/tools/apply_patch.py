...
def parse_legacy_patch(patch: str) -> List[FileChange]:
    ...
    # Split into file operations - capture file path only on same line (no newlines)
    file_pattern = r"\*\*\* (Add|Delete|Update) File: ([^\n]+)"
    
    for file_match in re.finditer(file_pattern, content):
        op_type = file_match.group(1).lower()
        file_path = file_match.group(2).strip()
        
        # Get content after header (everything until next file operation or end)
